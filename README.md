...
- **Action-only steps:** use the 2-arg `makeStep(trigger, action)` to omit speed segments entirely (both default to `makeNoSpeedSegment()`). Chain manual moves with consecutive `makeNoTrigger()` steps — forward, backward, turn — and the runner advances **without stopping** between them. When the next step has a real trigger (line, distance, rules), motors stop and wait as usual.
  ```cpp
  const RouteStep PLAN[] = {
    makeStep(makeNoTrigger(), makeActionForward(200, stopByDistance(2.0f))),
    makeStep(makeNoTrigger(), makeActionBackward(200, stopByDistance(1.0f))),
    makeStep(makeNoTrigger(), makeActionTurnLeft(200, stopByTime(400))),
  };
  ```
- **SpeedA / SpeedB:** after `Action` ends, runner enters line-follow mode for SpeedA then optionally SpeedB. Use the 2-arg `makeStep(trigger, action)` for action-only steps, or the 3-arg `makeStep(trigger, action, speedA)` to omit SpeedB. With a finite SpeedA stop (`stopByTime` / `stopByDistance`), the robot **stops** and waits for the next step’s trigger; with `makeSpeedSegment(speed)` (until-next-trigger), it keeps moving until that trigger fires. If SpeedA uses until-next-trigger in a 4-arg step, SpeedB is skipped when the next step’s trigger fires.
